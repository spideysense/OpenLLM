...
function injectSearch(messages, query, results) {
  ...
  if (hasSystem) return [{ ...messages[0], content: messages[0].content + searchBlock }, ...messages.slice(1)];
  const now = new Date();
  const dateStr = now.toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
  const timeStr = now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', timeZoneName: 'short' });
  return [{ role: 'system', content: `You are a helpful private AI assistant. Today is ${dateStr}, ${timeStr}.${searchBlock}` }, ...messages];
}
...
export default async function handler(req) {
  ...
  if (!parsed.hostname.endsWith('.runonaspen.com') && parsed.hostname !== 'runonaspen.com') return jsonError('tunnelUrl must be runonaspen.com domain', 403);

  const upstream = `${tunnelUrl.replace(/\/+$/, '')}/v1/chat/completions`;
  const upHeaders = { 'Content-Type': 'application/json', 'User-Agent': 'Aspen-Web-Proxy/1.0', ...(apiKey ? { 'Authorization': `Bearer ${apiKey}` } : {}) };
  ...
  let upstreamRes;
  try {
    upstreamRes = await fetch(upstream, { method: 'POST', headers: upHeaders, body: JSON.stringify({ model: model || 'llama3', messages: enrichedMessages, stream }) });
  } catch (err) { return jsonError(`Could not reach tunnel: ${err.message}`, 502); }

  if (!upstreamRes.ok) { const text = await upstreamRes.text().catch(() => ''); return jsonError(`HTTP ${upstreamRes.status}: ${text}`, upstreamRes.status); }

  if (!stream) { const json = await upstreamRes.json(); return new Response(JSON.stringify(json), { status: 200, headers: { ...corsHeaders(), 'Content-Type': 'application/json' } }); }

  return new Response(upstreamRes.body, { status: 200, headers: { ...corsHeaders(), 'Content-Type': 'text/event-stream', 'Cache-Control': 'no-cache', 'X-Accel-Buffering': 'no' } });
}

function jsonError(msg, status) { return new Response(JSON.stringify({ error: msg }), { status, headers: { ...corsHeaders(), 'Content-Type': 'application/json' } }); }
