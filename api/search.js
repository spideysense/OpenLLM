/**
 * /api/search — Web search for Aspen's tool-use loop
 *
 * Primary: Serper.dev (Google results, 2500 free/month, optional SERPER_API_KEY env var)
 * Fallback: DuckDuckGo Instant Answer API (free, no key, limited but always works)
 *
 * Returns: { results: [{ title, url, snippet }], source: 'serper'|'ddg' }
 */
...
export default async function handler(req) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.answerBox?.answer) {
          results.push({ title: 'Direct answer', url: '', snippet: data.answerBox.answer });
        } else if (data.answerBox?.snippet) {
          results.push({ title: data.answerBox.title || 'Direct answer', url: data.answerBox.link || '', snippet: data.answerBox.snippet });
        }
        // Knowledge graph
  ...
  if (results.length === 0) {
    try {
      ...
      const res = await fetch(ddgUrl, { headers: { 'User-Agent': 'Aspen/1.0' } });
      if (res.ok) {
        const data = await res.json();
  ...
  return new Response(JSON.stringify({ results: results.slice(0, 6), source, query }), {
    status: 200,
