const { allowed, join, newReceipt, remove, status } = require('../src/cloud/waitlist.cjs');
...
export default async function handler(req, res) {
  ...
  if (req.method === 'PATCH') {
    if (typeof body.receipt !== 'string' || !/^[A-Za-z0-9_-]{43}$/.test(body.receipt)) return res.status(400).json({ error: 'Your private waitlist link is missing.' });
    try {
      const ip = String(req.headers['x-forwarded-for'] || req.socket?.remoteAddress || 'unknown').split(',')[0].trim();
      if (!await allowed(ip, 'status')) return res.status(429).json({ error: 'Please check again in an hour.' });
      return res.status(200).json({ ok: true, waitlist: await status(body.receipt) });
    } catch { return res.status(503).json({ error: 'Your place is saved. We couldn’t load your position just now.' }); }
  }
  ...
  try {
    ...
    const referral = typeof body.ref === 'string' && /^[A-Za-z0-9_-]{16}$/.test(body.ref) ? body.ref : '';
    await join(email, source, receipt, referral);
