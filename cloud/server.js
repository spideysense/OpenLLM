...
app.get('/health', (req, res) => {
  res.json({ status: 'ok', service: 'aspen-cloud', version: '0.1.0' });
});
...
app.post('/tunnel/register', (req, res) => {
  try {
    ...
    const stableUrl = `${process.env.API_BASE_URL || 'https://api.aspen.com'}/t/${tunnelId}`;
...
app.all('/t/:tunnelId/*', (req, res) => {
  ...
  if (!tunnel) {
    return res.status(502).json({
      error: { message: 'Tunnel not found or offline. Make sure Aspen is running.', type: 'tunnel_error' }
    });
  }
  ...
  if (staleMinutes > 5) {
    return res.status(502).json({
      error: { message: 'Aspen appears offline (no heartbeat in ' + Math.round(staleMinutes) + ' min).', type: 'tunnel_offline' }
    });
  }
  ...
  proxyReq.on('error', (err) => {
    res.status(502).json({
      error: { message: 'Could not reach local AI. Make sure Aspen is running.', type: 'proxy_error', detail: err.message }
    });
  });
...
if (!process.env.VERCEL) {
  app.listen(PORT, () => {
    console.log(`🐻 Aspen Cloud running on port ${PORT}`);
