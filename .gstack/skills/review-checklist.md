...
- **Missing CHANGELOG entry for user-facing changes.** If the diff adds, changes, or fixes anything a user would notice — new feature, UI change, bug fix, dark mode fix, new page — CHANGELOG.md MUST have a corresponding entry with: feature name in plain English, description of the user benefit, and the date (YYYY-MM-DD). Flag as incomplete if missing.
