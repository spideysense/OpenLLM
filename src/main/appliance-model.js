...
async function ready({ signal } = {}) {
  ...
  try {
    ...
    if (!model) {
      ...
      const recommended = models.getRecommendation(
      ...
      if (
        !installed.some(
          (m) => require('./model-id').normalize(m.name) === require('./model-id').normalize(model)
        )
      ) {
        status('downloading', model);
        const result = await models.pullModel(model, () => {}, { signal });
