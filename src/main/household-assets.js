...
function serve(req, res) {
  ...
  else if (/^\/household\/assets\/[a-zA-Z0-9_-]+\.(js|css)$/.test(req.url)) name = req.url.slice('/household/'.length);
  else return false;
  const file = path.resolve(__dirname, '../../build-household', name);
  if (!fs.existsSync(file)) { res.writeHead(503); res.end('Build the household application before starting Aspen.'); return true; }
  const type = name.endsWith('.js') ? 'text/javascript' : name.endsWith('.css') ? 'text/css' : 'text/html';
  const content = fs.readFileSync(file);
  res.writeHead(200, {
    'Content-Type': type + '; charset=utf-8', 'Content-Length': content.length, 'Cache-Control': 'no-store',
