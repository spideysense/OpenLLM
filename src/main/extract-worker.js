...
if (!isMainThread) {
  require('./file-extract')
    .extractText(workerData)
    .then((result) => parentPort.postMessage(result))
    .catch(() => parentPort.postMessage({ ok: false, error: 'Document extraction failed' }));
}
let running = 0;
function extract(input) {
  if (running >= 2)
    return Promise.reject(
      new Error('Two documents are being imported. Try again when they finish.')
    );
  running++;
